...
> 时间基准说明（二轮 Review 后补充）：主动交互的所有时间参数（轮询间隔、冷却、频率窗口）均使用**实时时间**，不受游戏时间缩放影响。理由：主动交互面向玩家体验，玩家感知的是实时时间；若使用游戏时间，时间加速场景（如日程系统 1秒=1小时）会导致 NPC 在玩家视角疯狂触发主动交互。
...
```
Prompt 结构（缓存友好排列，二轮 Review 后修正）：
┌─────────────────────────────┐
...
> 修正理由：输出约束层在 4.6.1 中定义为"不可覆盖/不可截断"，同一 Phase 内内容固定（Phase 1 纯文本 vs Phase 2+ JSON Schema）。移入稳定前缀可最大化 prefix caching 命中率。
...
- `PrewarmConnectionTimeout: float = 30.f`（二轮 Review 后补充）：空闲连接超时释放（秒），玩家离开感知范围后计时，超时则关闭连接释放服务端资源
...
- `MinEmotionBroadcastInterval: float = 0.5f`（二轮 Review 后补充）：最小广播间隔（秒），即使阈值触发也不会高于 2Hz 广播频率。20 NPC 同时活跃时峰值带宽 ≈ 20×2Hz×28bytes ≈ 1.1KB/s，可接受
