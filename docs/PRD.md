...
- [ ] CustomProvider：开发者可通过实现 `ILLMProvider` 接口接入任意自定义 endpoint
...
- FR-13：记忆冲突解决：写入前检索相似记忆（有 Embedding 用向量检索，无 Embedding 降级为 FTS5 BM25 相似度），Top-K=5 候选，相似度阈值默认 0.75（可配置），异步 LLM 判断 ADD/UPDATE/MERGE/COEXIST/SUPERSEDE（计入**记忆维护池**并发配额，不占用对话池；维护池满时冲突解决降级为 ADD）；LLM 判断失败时降级为 ADD；COEXIST 的矛盾记忆检索时全部返回并标记 `Contradicted` 标签，由 Prompt 层呈现矛盾供 LLM 自行判断
...
### 安全系统
- FR-39：`InputSanitizer` 覆盖 3 类攻击：直接提示 / 社会工程 / 指令覆盖
- FR-40：`OutputValidator`：JSON Schema 校验 + 动作白名单 + 人设边界检测 + system prompt 泄露检测
...
- NFR-3：同帧最大并发 LLM 请求 ≤ 3，采用双池调度：对话池（高优先，默认并发2）+ 记忆维护池（后台，默认并发1）；远距离 NPC 调用频率降至 1/5（LOD）；Phase 1 起通过简单计数器限流，Phase 5 NpcScheduler 扩展为优先级队列调度
- NFR-4：情景记忆上限 200 条/NPC，长期记忆 SQLite 单表 < 10MB/NPC
- NFR-5：插件零**项目代码**依赖；引擎侧仅依赖标准模块与标准插件。文档需同时声明“直接依赖”和“传递依赖”（例如 SmartObjects 传递拉入 GameplayAbilities/TargetingSystem/MassEntity）
...
| SmartObjectsModule | 环境交互 | 需在 .uproject 启用；存在传递依赖（GameplayAbilities/TargetingSystem/MassEntity 等） |
...
- 直接依赖：Core, CoreUObject, Engine, AIModule, GameplayStateTreeModule, SmartObjectsModule, HTTP, WebSockets(可选), Json, JsonUtilities, GameplayTags, SQLiteCore
- 传递依赖（由引擎模块拉入）：GameplayAbilities, TargetingSystem, MassEntity, WorldConditions, NavigationSystem, PropertyBindingUtils, RHI 等
...
| DelayMaskingMontages | TMap\<EDelayStrategy, TSoftObjectPtr\<UAnimMontage\>\> | FR-33 | 1 |
...
| 轨道 | Phase | 内容 | 交付物 |
|------|-------|------|--------|
| Foundation | Phase 1 MVP | 基础对话 | OpenAI Provider + UAINpcComponent + AAINpcController + StateTree + 对话气泡 UI + 过渡动画 |
| Foundation | Phase 2 | 感知与行为 | NpcEventSubsystem + 结构化 JSON + SmartObject 执行 + 多 Provider（Anthropic/DeepSeek/Local）+ SSE 流式 |
| Foundation | Phase 3a | 记忆存储与检索 | 三层记忆 + Stanford 检索（IRelevanceScorer）+ 分段衰减 + 选择性写入 + SQLite 持久化 + 冲突解决 + 主动遗忘 |
| Foundation | Phase 3b | 反思与压缩 | 反思机制 + Evidence Pointers + 记忆合并归档 + MemoryType + LinkedMemoryIds |
| Foundation | Phase 4 | 情感/关系/安全 | VAD 情感 + 评价链 + OCEAN 人格 + 关系模型 + InputSanitizer + OutputValidator |
| Foundation | Phase 5 | 打磨与工具 | PersonaEditor + MemoryDebugger + NpcScheduler + 测试框架 + 示例项目 |
| Immersion Pack（可独立开关） | Phase 6 | 沉浸感增强 | 自主行为循环 + 主动交互触发 + NPC 间社交 + 流式首 Token <500ms + 情感外化接口 |
