...
- FR-32A：移动可达性与失败语义：所有需要移动到目标、槽位或位置的动作，在 Prompt 注入前和执行前都必须经过 `NavigationReachability` 检查；使用 UE `NavigationSystem`、NavMesh、`AAIController::MoveTo` / PathFollowing 结果作为权威，不允许 LLM 输出路径点、NavLink、瞬移或碰撞穿透指令。失败原因至少区分 InvalidTarget、MissingNavData、OffNavMesh、NoPath、PartialPathRejected、BlockedByStaticGeometry、BlockedByDynamicObstacle、Aborted、Timeout。墙、不可通行土包、空气墙本质上都归入导航/碰撞阻挡；如果项目方用 NavLink 或自定义区域把地形配置为可通行，才可视为可达。失败后必须释放已占用的 SmartObject 槽位，广播 ActionFailed/MovementFailed 事件，并由 StateTree 选择替代动作、对话说明或回到 Idle；静态不可达不重试，动态阻挡最多允许一次短延迟重新规划，禁止无限重试。
