...
- [ ] 移动/SmartObject 前置可达性：对需要移动到目标、槽位或位置的动作，执行前必须使用 UE Navigation/PathFollowing 验证可达性；不可达目标不得作为可执行动作注入给 LLM，玩家明确要求不可达目标时必须转成可解释的失败反馈
- [ ] 寻路失败可诊断：墙、土包、空气墙、动态阻挡、目标离开 NavMesh 等情况必须返回结构化失败原因，不得静默站桩、无限重试、瞬移兜底或让 LLM 直接规划绕路
- [ ] "裁判"架构：LLM 只建议动作，StateTree 验证动作合法性、目标合法性和路径可达性后才执行
...
- FR-32："裁判"架构：LLM 只建议，StateTree 验证动作合法性、目标合法性、路径可达性和当前 NPC 状态后才执行；Phase 2 起 `FStateTreeTask_ExecuteSmartObject` 内联基础白名单校验（仅允许 SmartObject 动态注入列表中的动作），Phase 4 抽出 `IActionValidator` 接口由 OutputValidator 实现完整校验
- FR-32A：移动可达性与失败语义：所有需要移动到目标、槽位或位置的动作，在 Prompt 注入前和执行前都必须经过 `NavigationReachability` 检查；使用 UE `NavigationSystem`、NavMesh、`AAIController::MoveTo` / PathFollowing 结果作为权威，不允许 LLM 输出路径点、NavLink、瞬移或碰撞穿透指令。失败原因至少区分 InvalidTarget、MissingNavData、OffNavMesh、NoPath、PartialPathRejected、BlockedByStaticGeometry、BlockedByDynamicObstacle、NoProgress、Aborted、Timeout。墙、不可通行土包、空气墙本质上都归入导航/碰撞阻挡；如果项目方用 NavLink 或自定义区域把地形配置为可通行，才可视为可达。失败后必须释放已占用的 SmartObject 槽位，广播 ActionFailed/MovementFailed 事件，并由 StateTree 选择替代动作、对话说明或回到 Idle；静态不可达不重试，动态阻挡最多允许一次短延迟重新规划，禁止无限重试。
- FR-32B：卡住观测与可恢复性上下文：LLM 不自行判断 NPC 是否卡住，也不判断能否解决。移动执行器必须把 PathFollowing 失败、距离目标长期无进展、目标/槽位状态变化和重规划结果整理为 `MovementFailureObservation`，由本地规则标注 RecoverableByRetry、RecoverableByAlternativeAction、RecoverableByWorldChange 或 UnrecoverableForCurrentAction。PromptBuilder 下一轮只注入这份结构化事实、失败原因、可用替代动作和玩家可见摘要；LLM 只能基于这些事实生成角色化解释、选择已给出的替代动作或请求玩家/世界状态变化，不能覆盖可恢复性结论。
...
- NFR-12：移动可达性检查必须有界：只对当前候选动作/目标执行路径查询，不做全地图扫描；候选数量、查询半径和 MoveTo 超时必须可控；批量候选的同步查询不得制造可感知 GameThread 卡顿
...
| NavigationSystem | 移动可达性预检、NavMesh 查询、PathFollowing 失败归因 | — |
...
- 直接依赖：Core, CoreUObject, Engine, AIModule, GameplayStateTreeModule, NavigationSystem, HTTP, WebSockets(可选), Json, JsonUtilities, GameplayTags, SQLiteCore, SmartObjectsModule
- 传递依赖（由 SmartObjects 引擎模块拉入）：GameplayAbilities, TargetingSystem, MassEntity, WorldConditions, PropertyBindingUtils, RHI 等
...
- 自研通用寻路/绕路规划系统：移动动作复用 UE Navigation、NavMesh、PathFollowing、NavLink 和项目方已有移动组件；插件只做候选可达性、失败归因和行为链路回退，不替代引擎导航系统
...
| LLM 幻觉（不合理行为） | 🟡 中 | 动作白名单 + SmartObject 合法列表约束 + 路径可达性预检 + "裁判"架构验证 |
| NPC 移动卡死或空气墙阻挡后无反馈 | 🟡 中 | NavigationReachability 预检 + PathFollowing 失败归因 + ActionFailed/MovementFailed 事件 + 可视化验收覆盖墙/土包/空气墙/动态阻挡 |
...
- 移动动作失败可解释率：可视化场景中墙、土包、空气墙、动态阻挡、目标离开 NavMesh 五类失败均能在 2 秒内停止无效移动并产生明确失败原因、玩家可见反馈或替代动作
