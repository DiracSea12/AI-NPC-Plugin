...
基于上述 9 篇论文，对 SDD v1.5 Phase 6 设计的补充改进建议：
...
#### 4.5.8 Phase 6 二轮 Review 补充发现（2026-03，独立 Agent 二轮调研）

> 由独立 Agent 对四份文档进行跨文档一致性审查 + 网络调研，以下为经逐条核实后采纳的增量发现。

**新发现的论文/产品（文档此前未覆盖）：**

| 论文/产品 | 核心贡献 | 对 Phase 6 的影响 |
|-----------|----------|------------------|
| **Sentipolis** (2601.18027) | 情感感知 Agent 社交模拟，提出"双速情感动态"（快速反应+慢速演化） | 当前 VAD 衰减只有单速率，未来可增加快/慢双时间常数 |
| **Deflanderization** (2510.13586) | LLM NPC "过度角色扮演"问题——NPC 过于入戏导致无法完成功能性任务 | FR-43 主动交互的 OutputValidator 应增加"任务完成度"检测维度 |
| **VR NPC 可信度评估** (2507.10469) | AI NPC 可信度 6.67/10，情感/人格维度得分最低 | 验证 FR-46 情感外化的重要性——情感是当前 AI NPC 短板 |
...
| **Speculative Decoding** (2511.21699) | 小模型预测 + 大模型并行验证，延迟降低 2-3x | FR-45 未提及，对 LocalProvider 场景有显著优化潜力 |
| **Semantic Caching** | 语义相似查询返回缓存响应，减少 30-50% LLM 调用 | FR-44 NPC 社交中重复场景（每天打招呼）可用语义缓存 |
| **EvoEmo** (2509.04310) | 进化 RL 优化动态情感表达策略 | 长期方向：用 RL 替代规则驱动的情感系统 |
| **Affective Inertia** (2026.01) | LLM Agent "情感抖动"问题——共情/冷漠/和解交替出现 | MinEmotionDuration 只是动画层平滑，情感状态层需要类似"情感惯性"机制 |

**经核实采纳的跨文档一致性修复（已写入 SDD v1.7 + PRD v1.7）：**

| # | 改进项 | 问题来源 | 已写入SDD章节 |
|---|--------|----------|--------------|
| 18 | NPC↔NPC 关系独立存储表 `npc_relationships` | 跨文档一致性：relationships 表只有 player_id | 6.5 新增 |
| 19 | 主动交互时间基准明确为实时时间 | 跨文档一致性：30s 未区分游戏/实时时间 | 4.9.2 主动交互 |
| 20 | 输出约束层移入 Prompt 稳定前缀 | 技术审查：与 4.6.1 "不可截断"定义矛盾 | 4.9.4 流式首Token |
| 21 | 社交搜索半径 `SocialSearchRadius = 2000cm` | 技术审查："附近"无具体距离 | 4.9.3 NPC社交 |
| 22 | `MaxConcurrentSocial` 明确为全局限制 | 技术审查：作用域不明确 | 4.9.3 NPC社交 |
| 23 | 情感广播最小间隔 `MinEmotionBroadcastInterval = 0.5s` | 技术审查：高频广播带宽风险 | 4.9.5 情感外化 |
| 24 | 社交模板 `FString` → `FText`（本地化） | 技术审查：硬编码字符串不支持多语言 | 4.9.3 NPC社交 |
| 25 | 连接预热超时释放 `PrewarmConnectionTimeout = 30s` | 技术审查：空闲连接资源泄漏 | 4.9.4 流式首Token |

**标记为未来增强（不在当前版本实现）：**

| 方向 | 来源 | 理由 |
|------|------|------|
| OCEAN 人格影响日程偏好权重 | 一轮 Review 建议 | 增强功能，静态日程表作为 MVP 足够 |
| 粗粒度时间槽 + LLM 细化两层日程 | 一轮 Review 建议 | 增强功能，可作为编辑器工具后续添加 |
| 情感基础表情/叠加分层 | 一轮 Review 建议 | 项目方可在动画蓝图中自行实现分层 |
