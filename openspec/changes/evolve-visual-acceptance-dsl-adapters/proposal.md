...
- Phase 2.8 只增加 internal adapter registry/seam，用于内置 fixture 准备、世界事件、动作执行、角色驱动和 observation 采集；不发布 project adapter public API、public registry 或 extension interfaces。
...
- Phase 2.9 起才暴露经过验证的项目扩展接口，让下游项目能接入自己的 NPC actor、动作系统、世界事件、感知系统和 observation provider，而不是修改 AI-NPC 核心 runner。
...
- `visual-acceptance-adapters`：Phase 2.8 覆盖 fixture、event、action、character、observation 的 internal adapter registry/seam；Phase 2.9 才覆盖 public extension interfaces。
