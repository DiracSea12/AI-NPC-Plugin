...
Phase 2.9B 不能只发布空 marker interface。公开 seam 仍只保留 `FixtureResolver`、`ActionAdapter`、`ObservationProvider` 三个 category，但每个 category 必须有一个窄行为方法，并且方法签名必须由最小 request/result value structs 表达：
- `FAINpcVisualFixtureResolveRequest`：`UWorld* World`、test id、run id、adapter id、fixture kind、actor class、actor tag、target ref name。`FAINpcVisualFixtureResolveResult`：success flag、resolved `TWeakObjectPtr<AActor>`、target ref name、diagnostic。
- `FAINpcVisualActionExecuteRequest`：test id、run id、step index、adapter id、action name、target ref name、resolved `TWeakObjectPtr<AActor>`。`FAINpcVisualActionExecuteResult`：accepted/succeeded flag、attempt diagnostic、failure reason；不得包含 observation record 或 observation writer。
- `FAINpcVisualObservationSampleRequest`：test id、run id、adapter/provider id、observation name、source actor ref。`FAINpcVisualObservationSampleResult`：success flag、typed observation record、diagnostic；sampling method 必须来自 declaration。

`phase-2-9b-start-readiness.md` 中的 "2.9B public API contract table" 是本阶段签名级真源；开发代理不得另起方法名、字段名、返回类型或额外 public API 文件。

这些新增 value structs 只能放进既有 public extension header，不新增 public API 文件、不新增 category、不引入 Blueprint/UObject-owning interface。adapter factory 仍返回对应 category instance；runner 只能按 descriptor category 使用对应窄接口，不能 downcast 到项目类型。

Phase 2.9B 的固定 project door identifiers 只允许作为 focused automation/test descriptor data 出现。Core parser/runner 不得把 `project.door.*`、`Interact` 或 door capability 写成 built-in domain branch、built-in observation constant 或 hardcoded schema value；core 只做字段形状、category、adapter id presence、observation declaration lookup 和 target ref 解析。2.9B 不从 `actionName` 推导 capability，也不实现 `Interact` -> `projectAction.doorInteract` 之类的通用或 door-specific matcher；本阶段没有 scenario field 或 descriptor field 用来表达 required action capability，所以 action extension validation 只确认 adapter id 属于 `ActionAdapter` category。具体 action name 是否被支持由 project action adapter 在 `StepExecution` 返回 accepted/rejected diagnostic。真实 visible door actor/scenario 闭环留到 2.9C。

Phase 2.9B 的 fixture capability 规则不是可选发挥：`kind: "existingActor"` 在 `ExtensionDeclaration` 必须绑定到 `FixtureResolver` descriptor，且 descriptor `Capabilities` 必须包含 `existingActor.classTag`。未注册 adapter id、错误 category、缺少该 capability 都必须在 `ExtensionDeclaration` 失败，不能拖到 runtime actor lookup。

Phase 2.9B 的 target ref 固定为 `fixture.actor`：existing-actor fixture 在 per-run fixture refs 中只绑定这一个 resolved actor ref；`project.action.execute.payload.targetRef` 必须引用它。Final assertion sampling 也使用同一个 per-run `fixture.actor` 作为 `FAINpcVisualObservationSampleRequest::SourceActor`；如果该 ref 未绑定或已失效，final assertion 失败，observation provider 不得自造 source。不要发明 `fixture.door`、`fixture.target` 或按 test id 推导 ref name。这个 ref 随 scenario end/world teardown 失效，不能进入 module-level registry 或跨 run cache。

Phase 2.9B 的 class lookup 只允许当前 world 中已加载 native class。`actorClass` 必须是 `/Script/<Module>.<ClassName>`；实现不得用 load path 兜底，不得接受 Blueprint generated class、short name、soft object path 或 object reference；valid-shaped native class path 如果不能解析为当前已加载 native class，必须在 `RuntimeStartup` 失败且不得进入 actor enumeration；actor match 必须是 `Actor->GetClass() == ResolvedClass`，不得使用 subclass/`IsA`/`IsChildOf` 匹配，并且只在当前 scenario world 枚举 loaded、valid、non-pending-kill actors。

Phase 2.9B 最小 diagnostics 不新建 report/framework，但需要复用并扩展现有 `FAINpcVisualTestStepDiagnostic` carrier。先复用现有 `StepIndex`、`StepType`、`Status`、`FailureReason`、`FailureCategory`、`ObservationName`、`SourceKind`、`SourceId`；新增字段只限 `Stage`、`AdapterCategory`、`AdapterId`、`OwnerModuleName`、`ActorClass`、`ActorTag`、`TargetRef`、`ActionName`、`FieldName`、`Capability`。没有 step index 的 startup/declaration failure 可以使用 `INDEX_NONE`。这些是 available-field diagnostics，不要求每个阶段伪造不知道的字段；也不要靠自由格式 failure string 冒充结构化字段。

`phase-2-9b-start-readiness.md` 中的 "2.9B diagnostic field matrix" 是本阶段 stage-to-field 真源；任何字段不在矩阵中可用时不得用空字符串或猜测值冒充结构化事实。
...
Phase 2.9B 开始，public adapter descriptor 的 observation declarations 不能再只是 observation name 字符串。最小 declaration struct 名称固定为 `FAINpcVisualObservationDeclaration`，字段为 `ObservationName`、`ValueType`、`SourceKind`、`SamplingMethod`、`Capability`、`bRequiresSourceObjectPath`、`bRequiresSourceClass`。`FAINpcVisualAdapterDescriptor::ObservationDeclarations` 必须改为该 typed struct array；provider id 使用 descriptor `AdapterId`，不新增第二个 provider id 字段。每条 declaration 的 `Capability` 字段就是该 observation 的 required capability 来源，并且必须非空且精确存在于所属 `ObservationProvider` descriptor 的 `Capabilities`。capability（例如 `observation.project.door.isOpen`）不能替代 runtime observation name（例如 `project.door.isOpen`）。2.9B 的 `project.door.isOpen` 正向 final assertion 只能作为 focused automation/test-owned state-read 样例验证 declaration 和 final assertion 机制；不得在 2.9B 新增 visible door actor、map、scenario 或用户可见 door 示例。

Scenario static parse 只负责引用形状：内置 observation 继续走 built-in known set，`project.<domain>.<name>` 只验证非空 namespace 形状。实现落点必须拆在 C++ parser helper 上：把现有 `IsKnownVisualObservationName` 收窄为 built-in 判断，再新增 project observation shape 判断；不得关闭 built-in unknown observation validation。descriptor load 后运行一次 declaration-aware validation，确认 referenced project observations 被 registered observation provider 声明，并且 declaration 的 value/source/sampling/capability 满足当前 stage。PowerShell 只能核对 C++ contract 和 artifact，不允许维护第二套 observation 白名单。
