...
- 提供扩展接口，让项目模块可接入 combat、inventory、quest、door、多模态视觉、听觉等系统，而不用改核心 runner。
...
每种 step type 都有明确 payload contract，而不是开放对象：
...
Phase 2.7 就必须拒绝 unknown top-level field、legacy top-level field、unknown step type、缺失必填字段和 unresolved prompt variable。Phase 2.95 再扩展到完整 nested unknown-field rejection、observation declaration validation、namespace validation 和 assertion operator validation。
...
Phase 2.7 可以让 adapter 保持 internal 且范围有限，但 runner 从第一版 DSL 起就必须通过 adapter-like seam 分发 event、action、fixture 和 observation 行为。不能先写一个套着 DSL 外壳的 `if (TestId == ...)` 或 `if (ActionType == ...)` runner。
...
Phase 2.8 先把内置 dialogue、event、SmartObject、fixture、character driver、observation 逻辑拆成 internal adapters，并保持 API 私有。Phase 2.9 再暴露经过验证的最小 public interfaces 给项目模块；public API 不得一次性把所有未来想象中的 extension surface 都冻结出去。

registry 的推荐落点是 scenario/runtime context 持有的 registry view：模块级 registry 只保存 adapter factory 或 descriptor，实际 adapter instance 和 observation store 必须绑定当前 world/scenario/run。禁止用跨 run 的 static mutable adapter instance 或 static observation store。执行前校验 adapter id，同 category duplicate id 直接拒绝，并定义 module startup/shutdown、world teardown 和 scenario end 下的注册/注销或失效时机。不能让另一个 world、PIE instance 或上一次 run 的 stale adapter 影响当前 scenario。
...
第一版 assertion grammar 只支持声明过的 operator：`all`、`any`、`anyOf`、`equals`、`notEquals`、`greaterThan`、`greaterThanOrEqual`、`lessThan`、`lessThanOrEqual`、`exists`、`notExists`。Operator 必须声明 expected value type 和 missing-value behavior。`notExists` 必须要求 observation provider/source 已就绪且采样窗口已覆盖，否则不能用缺失 observation 当成功证据。Unsupported operator 直接 validation fail。
...
2.7/2.8/2.9 之后，Phase 2.95 收紧完整 nested schema validation、unknown-field rejection、adapter ID validation、prompt variable resolution、observation declaration checks、namespace rules、negative fixture tests、provider evidence checks 和 per-step diagnostics。
