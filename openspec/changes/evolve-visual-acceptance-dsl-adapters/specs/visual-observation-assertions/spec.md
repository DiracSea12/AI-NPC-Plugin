...
#### Scenario: 比较 numeric observation
- **WHEN** scenario expectation 比较 distance 或 duration 之类 numeric observation
- **THEN** assertion 声明一个受支持 comparison operator 和 expected value；失败时报告 actual value、expected value、operator 和 source observation timestamp

#### Scenario: 使用不支持的 assertion operator
- **WHEN** scenario 使用不在支持集合内的 assertion operator；支持集合为 `all`、`any`、`anyOf`、`equals`、`notEquals`、`greaterThan`、`greaterThanOrEqual`、`lessThan`、`lessThanOrEqual`、`exists`、`notExists`
