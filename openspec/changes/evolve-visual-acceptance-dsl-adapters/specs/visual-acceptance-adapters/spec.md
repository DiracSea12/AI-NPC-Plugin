...
#### Scenario: Phase 2.9 驱动 project character
- **WHEN** 项目为自己的 `ACharacter` subclass 注册 character driver
...
- **WHEN** 两个 adapters 在同一 category 注册同一个 id
- **THEN** registration 或 validation fail，而不是静默替换其中一个 adapter
