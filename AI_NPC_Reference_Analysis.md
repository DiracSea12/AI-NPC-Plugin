...
**结论**：Phase 6 的五项功能（自主行为/主动交互/NPC社交/流式首Token/情感外化）填补了所有竞品的共同空白，是从"AI 聊天机器人"到"AI 原住民"的关键跨越。
...
| # | 改进项 | 来源 | 影响系统 |
|---|--------|------|----------|
| 17 | 自主行为循环（日程表驱动 StateTree） | Stanford Generative Agents | 行为 |
| 18 | 主动交互触发（关系/记忆/情感条件→NPC发起） | 主动交互研究 | 行为 |
| 19 | NPC间社交协议（轻量NPC↔NPC对话） | Stanford Generative Agents | 社交 |
| 20 | 流式首Token优化（<500ms可见） | 玩家反馈调研 | 通信 |
| 21 | 情感外化接口（VAD→动画蓝图参数） | 情感外化研究 | 情感 |
...
7. **自主行为是沉浸感的分水岭（Phase 6 新增）** — Stanford Generative Agents 证明日程驱动的自主行为是可信度关键；所有现有竞品（Personica/Inworld/ACE/Convai）均未实现 NPC 主动交互，这是最大的差异化空白
8. **"活着"比"聪明"更重要（Phase 6 新增）** — NVIDIA ACE 技术先进但被批评"无灵魂"，说明技术能力不等于沉浸感。NPC 的自主生活节奏、主动社交行为、可见的情感表达，比对话质量更影响玩家的"活人"感知
